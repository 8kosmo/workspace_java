						...
						if(!imgChoice.equals("default")) {//이모타콘 메세지 일때
							...
							for(i=0;i<tc2.imo.imgFiles.length;i++) {
								if(tc2.imo.imgFiles[i].equals(imgChoice)) {
									StyleConstants.setIcon(attr1,new ImageIcon(tc2.imo.imgPath+tc2.imo.imgFiles[i]));
									try {
										tc2.sd_display.insertString(tc2.sd_display.getLength(),"\n", attr1);//이모티콘
									} catch (Exception e) {
										// TODO: handle exception
							...
							try {
								tc2.sd_display.insertString(tc2.sd_display.getLength(), "["+nickName+"]"+message+"\n", null);
							} catch (Exception e) {
								// TODO: handle exception
							}
						}
//						tc2.jtp_display.append("["+nickName+"]"+message+"\n");
					...
					case Protocol.CHANGE:{
						...
						for(int i=0;i<tc2.dtm_name.getRowCount();i++) {
							...
							tc2.jtp_display.setCaretPosition(tc2.sd_display.getLength());//줄바꿈처리
