...
/***********************************************************
 * 부모창에서 결정된 값(입력, aVO)
 * @param aVO 입력버튼을 부모창(AddressBook)에서 눌렀을 땐 null, 수정버튼을 눌렀을 땐 aVO는 DB에서 읽어온 값을 담고 있다.
 * @param title 부모창에서 선택한 버튼의 라벨담기
 * @param aBook 부모창의 원복 주소번지를 담는 변수
 * @param isEdit 자식창(SubBook)에서 입력콤퍼넌트(textfield)활성화 시키기 위한 값
